...
describe("makeAuditHook", () => {
  ...
  it("logs a tool invocation and always continues", async () => {
    ...
    const result = await makeAuditHook(log)({ tool_name: "Read" } as never, undefined, hookCtx);

    expect(info).toHaveBeenCalledWith({ event: "pre_tool_use", tool: "Read" }, "tool invocation");
